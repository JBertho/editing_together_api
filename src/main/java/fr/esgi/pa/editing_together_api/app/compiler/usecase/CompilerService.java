...
public class CompilerService {
    ...
    private final CompilerC compilerC;
    ...
    public String compileForC(DockerCompilation dockerCompilation, List<Snippet> snippetsCode, Project project) throws IOException, InterruptedException {
        ...
        compilerC.saveCode(sourceCode, 5);
        String imageName = "compile_in_c_" + new Date().getTime();

        String dockerError = compilerC.setupDocker(dockerCompilation, imageName);
        if (!dockerError.isEmpty()) {
            return dockerError;
        }
        return compilerC.runDocker(dockerCompilation, imageName);
